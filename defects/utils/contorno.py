...
def obtener_contorno_imagen(image, model, conf_threshold, f_epsilon=0.02):
    ...
    diff = np.diff(vertices, axis=1).reshape(-1)  # Cambio: reshape(-1) en lugar de reshape(4)
    
    # Crear el array ordenado
    ordered = np.zeros((4, 2), dtype=np.int32)
    
    # FIX: Usar np.argmin() y np.argmax() en lugar de operador < directo sobre arrays
    min_sum_idx = np.argmin(s)
    max_sum_idx = np.argmax(s)
