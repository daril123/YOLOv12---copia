...
class CornerCrackProcessor:
    """
    Procesador para las grietas corner (de esquina)
    """
    
    def __init__(self):
        """
        Inicializa el procesador de grietas corner
        """
        self.name = "grietas_corner"
    
    def measure_crack(self, crack_mask, corners=None, crack_img=None, bbox=None):
        """
        Mide las propiedades de una grieta corner en píxeles,
        ...
        if crack_img is not None:
            ...
            #cv2.imwrite("temp_corner_crack_analysis_roi.jpg", viz_img)
            ...
            if corners is not None and bbox is not None and contour_point is not None and edge_point is not None:
                try:
                    ...
                    #cv2.imwrite("temp_corner_crack_analysis_full.jpg", full_img)
                except Exception as e:
                    print(f"Error al crear visualización global para grieta corner: {e}")
                    import traceback
                    traceback.print_exc()
    ...
    def generate_report(self, image_name, cracks_data, output_dir):
        """
        Genera un informe simple de las grietas corner
        ...
        # Formato del informe
        report_path = os.path.join(output_dir, f"{image_name}_grietas_corner_report.csv")
        
        # Guardar como CSV
        df.to_csv(report_path, index=False)
        
        # También generar una versión en formato de texto para fácil visualización
        text_report_path = os.path.join(output_dir, f"{image_name}_grietas_corner_report.txt")
        
        with open(text_report_path, 'w') as f:
            f.write(f"REPORTE DE GRIETAS CORNER - {image_name}\n")
            f.write("="*50 + "\n\n")
    ...
    def process(self, detections, image, corners, zone_masks, image_name=None, output_dir=None):
        """
        Procesa todas las grietas corner detectadas
        
        Args:
            detections: Lista de detecciones de grietas corner
